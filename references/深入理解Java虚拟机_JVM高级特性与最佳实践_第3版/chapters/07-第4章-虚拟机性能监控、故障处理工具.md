...
中，JDK 11之前都无须独立下载，但是在商业环境中使用它则是要付费的[^section-4-2-note-1]。

- 正式支持工具：这一类工具属于被长期支持的工具，不同平台、不同版本的JDK之间，这类工具

可能会略有差异，但是不会出现某一个工具突然消失的情况[^section-4-2-note-2]。
...
- IBM的Support Assistant[^section-4-5-note-1]、Heap Analyzer[^section-4-5-note-2]、Javacore Analyzer[^section-4-5-note-3]、Garbage Collector

Analyzer[^section-4-5-note-4]适用于IBM J9/OpenJ9 VM。

- HP的HPjmeter、HPjtune适用于HP-UX、SAP、HotSpot VM。
- Eclipse的Memory Analyzer Tool[^section-4-5-note-5]（MAT）适用于HP-UX、SAP、HotSpot VM，安装IBM

DTFJ[^section-4-5-note-6]插件后可支持IBM J9虚拟机。
