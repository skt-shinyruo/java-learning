...
可能造成多个进程阻塞[^section-7-3-5-note-2]，在实际应用中这种阻塞往往是很隐蔽的。代码清单7-7演示了这种场景。
