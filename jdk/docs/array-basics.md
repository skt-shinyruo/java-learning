...
这篇文档的目标不是把数组相关语法零散罗列一遍，而是先把 8 个最容易混在一起的问题讲清楚：
...
在 Java 源码层面，并不存在一个等价的普通方法调用，例如：

```java
nums.get(0); // int[] 没有这个方法
```

`[]` 是 Java 语言直接支持的数组访问表达式。编译之后，JVM 会使用专门的数组读写指令，例如：
...
执行 `nums[i]` 时，大致会发生这些事情：

1. 检查 `nums` 是不是 `null`，如果是就抛 `NullPointerException`。
2. 检查 `i` 是否越界，如果越界就抛 `ArrayIndexOutOfBoundsException`。
3. 根据数组起始位置、元素大小和下标，定位到对应元素。
4. 完成读取或写入。

所以更准确的说法是：

- 宽泛地说，`[]` 是一种方便的访问语法
- 严格地说，`[]` 不是普通语法糖，而是语言内建的数组访问形式
...
至于数组对象在内存里的具体布局，Java 规范没有强制规定。不同 JVM 可以有不同实现。下面用常见的 HotSpot JVM 来帮助理解。
...
- 对象头：保存锁状态、GC 信息、hashCode 等运行时信息。
- 类型指针：说明这个对象是什么类型，例如 `int[]`。
- 数组长度：也就是 `arr.length` 的来源。
- 元素区：真正保存数组元素。
- 对齐填充：JVM 为了内存对齐可能补上的空白区域。

对于原始类型数组，元素值通常直接连续放在数组对象内部：

```text
arr  --->  [对象头][length=3][10][20][30]
```

对于对象数组，数组对象内部连续存放的是引用，不是对象本体：
...
- `int[]` 里放的是 `int` 值
- `String[]` 里放的是 `String` 引用
- `Object[]` 里放的是 `Object` 引用

二维数组也是同样的思路。Java 的二维数组本质上是“数组的数组”：
...
```text
matrix  --->  [对象头][length=2][ref to int[3]][ref to int[3]]
                                      |              |
                                      v              v
                                  [0][0][0]      [0][0][0]
```

所以 Java 数组不是 C 语言里那种纯粹的裸内存块。它是一个真正的对象，只是这个对象的尾部通常紧跟着一段连续的元素区域。
...
6. 数组的 `[]` 是语言内建的数组访问形式，不是普通方法调用的语法糖。
7. 常见 JVM 中，数组对象内部可以粗略理解成对象头、类型信息、长度和连续元素区。
8. `int[]` 存的是原始值，`Integer[]` 存的是对象引用，因此默认值、性能和 `null` 语义都不同。
9. 数组适合固定长度数据，`ArrayList` 更适合长度经常变化的数据。
